...
The Golay code $\cG \subset (\FF_2)^{24}$ is the span of the following (row) vectors:
\[	
	 \begin{array}{c}
...
Remarkably, the restriction $\cP\big|_V$ of the Parker loop reduces to a direct product since $\theta\big|_{V\times V}$ is identically zero. 
Moreover, the restriction $\cP\big|_W$ is the direct product $(\FF_2)^3 \times M$, where $M$ is the Moufang loop from Example~\ref{example:m16}.
